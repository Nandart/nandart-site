...
// Chão
const floorMaterial = new THREE.MeshStandardMaterial({ color: 0x111111, metalness: 0.4, roughness: 0.2 });
...
const glassMaterial = new THREE.MeshPhysicalMaterial({ color: 0x33aaff, transparent: true, transmission: 1, opacity: 0.6 });
const pedestalGema = (x, z) => {
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.6, 2, 32), pedestalMaterial);
  base.position.set(x, 1, z); scene.add(base);
  const gema = new THREE.Mesh(new THREE.OctahedronGeometry(0.5), glassMaterial);
  gema.position.set(x, 2.3, z); scene.add(gema);
};
pedestalGema(-10, -12); pedestalGema(-10, 12); pedestalGema(10, -12); pedestalGema(10, 12);

// Obras suspensas normais
